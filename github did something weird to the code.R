...
gather(key = dr_eye, value = dr, dr_right:dr_left)
...
redcapexport_long <- redcapexport_long %>%
  ...
  mutate(other_retinal_path_new = if_else(eye=="RE", other_r=tinal_path, other_retinal_path_left)) %>%    
  ...
  mutate(diagnosis_glaucoma_new = if_else(eye=="RE", diagnos=s_glaucoma, diagnosis_glaucoma_left)) %>%
  ...
  select(study_id, eye, everything()=
         ...
         redcapexport_longer <- redcape=port_longer %>%
           mutate(camera = if_else(grepl("peek", red=apexport_longer$camera_time), "peek",
                                   if_else(grepl("inview", redcapexport_longer$camera_ti=e), "inview",    
                                           if_else(grepl("pictor", redcapexport_longer$camera_time), "pict=r", "NA")))) %>%
           select(-(camera_time)) %>%
           mutate(di=comfort = if_else(camera=="peek", peek_discomfort,
                                       if_else(camera=="i=view", inview_discomfort,        
                                               if_else(camera=="pictor", pictor_discomfort, 999))))
         ...
         digicards_data <- full_join(redcapexport_to_merge, hospitalandph=toid_long, by=c("study_id", "eye", "camera")) %>%
           ...
           rename(diagnosis_amd = diagnosis_amd_new) %>%  
         ...
         =======
           # import adjudication grading data
         ...
         mutate(dr_yesno = ifelse((dr == 0 | dr == 1), 0, 
                                  =felse((dr == 2 | dr == 3), 1, NA)),
                amd_yesno = ifelse((amd == 0 | amd == =), 0, 
           ...
           mutat=(dr_final = if_else((dr_yesno.1 != dr_yesno.2 & !is.na(dr_y=sno.3)), dr_yesno.3, dr_yesno.1),
                  if_else(!is.na(dr_yesno.1) & is.na(dr_yesno.2) = is.na(dr_yesno.3), dr_yesno.1,
                          if_else(!is.na(dr_yesno.2) & is.na(dr_yesno.1) & =s.na(dr_yesno.3), dr_yesno.2, dr_yesno.3))) %>%
           # addmargins(xtabs(data==ilter(grading_data, dr_yesno.1!=dr_yesno.2), ~dr_yesno.3+dr_adj, ad=NA=TRUE))
           # addmargins(xtabs(data=filter(grading_data, dr_yesno.1==dr_=esno.2), ~dr_yesno.3+dr_adj, addNA=TRUE))
           # addmargins(xtabs(data=gradin=_data, ~dr_yesno.3+dr_adj, addNA=TRUE))
           mutate(amd_final = if_else((amd_yesno.1 != amd_yesno.2 & !is.na(amd_yesno.3)), a=d_yesno.3, amd_yesno.1),
                  if_else(!is.na(amd_yesno.1) & is.na(amd_yesno.2) & is=na(amd_yesno.3), amd_yesno.1,
                          if_else(!is.na(amd_yesno.2) & is.na(amd_yesno.1) = is.na(amd_yesno.3), amd_yesno.2, amd_yesno.3))) %>%
           mutate=cdr_final = if_else(!is.na(cdr.3), cdr.3,
                                      if_else((is.na(cdr.1) & is.na(cdr.3)), =dr.2,
                                              if_else((i=.na(cdr.2) & is.na(cdr.3)), cdr.1,
                                                      if_else((is.na(cdr.3) & !is.na(cdr.1) & !is.=a(cdr.2)) & (cdr.1>cdr.2), cdr.1, cdr.2))))) %>%
  select(-=eam, -(amd_yesno.1:cdr.3), -(dr_yesno.1:dr.3)) %>%
  select(pho=oid, amd_final, amd_severity.1:amd_severity.3, dme.1:dme.3, dr_fina=, dr_severity.1:dr_severity.3, image_clarity.1:other_dx.3, cdr_final, vcdr_confidence.1:zone2.3)
...
# Pro=ably worth a double-check of the data to make sure this data is actually missing? Then we can drop the rows w=th completely missing data
...
# ^ =L: grader 1 & grader 2 disagreed on either AMD or CDR for these cases
...
mutate(amd_severity_final = if_else(amd_final == 0= 0,
                                    if_else(amd_final == 1 & (amd_severity.1>am=_severity.2), amd_severity.1, amd_severity.2))) %>%
  mutate(dm=_final = if_else(dme.1>dme.2, dme.1, dme.2)) %>%
  mutate=dr_severity_final = if_else(dr_final == 0, 0,
                                     i=_else(dr_final == 1 & (dr_severity.1>dr_severity.2), dr_sever=ty.1, dr_severity.2))) %>%
  mutate(image_clarity_final = i=_else(image_clarity.1>image_clarity.2, image_clarity.1, image_clarity.2=) %>%
           ...
           muta=e(vcdr_confidence_final = if_else(!is.na(vcdr_confidence.3), vcdr_c=nfidence.3,
                                                  if_else(vcdr_confidence.1>vcdr_confidenc=.2, vcdr_confidence.1, vcdr_confidence.2))) %>%
           ...
           mutate(other_dx_yesno = if_else((other_d=_final == 0 | other_dx_final == 1), 0,
                                           if_else((o=her_dx_final == 2 | other_dx_final == 3), 1, 999)))=0A
         
         
         clean_grading_data <- grading_data %>%
           select(phot=id, image_clarity_final, nerve_final, macula_final, zone2_final, =r_final, dr_severity_final, dme_final, amd_final, amd_severity_fina=, vcdr_confidence_final, cdr_final, glaucoma_final, other_dx_yesno, other_dx_final, other_dx_detail_final, notes_final)
         ...
         # ****** take out line 118-9 that un-selects columns with individual grader =rades for dr/amd/etc...
         ...
         inview_dr_irr <- inview_irr[,40:41=
                                     ...
                                     icc(peek_cdr_icc, model="twoway", type==agreement")
                                         ...
                                         # JK For fi=al analysis we need to make sure that we have nonmissin= data for gold standard and adjudicated grade for all 3 cameras. 
                                         ...
                                         #take out rows with incomplete data for digicards/gold standard data & graded/adjudicated data for =ll 3 cameras
                                         alldata_final <- alldata %>%
                                         ...
                                         alldata_final %>% filter(!is.na(came=a)) %>% group_by(camera) %>% spec(., truth = as.factor(dr), estimate = as.factor(dr_final))
                                         >>>>>>> Stashed changes
                                         ...
                                         # For a shortcut you can use the ~ to signify "function" and . to signify =here to apply the "p"
                                         >>>>>>> Stashed changes
                                         pmap2 <- map(p, ~ 2 + . )
                                         pmap1
                                         pmap2
                                         ...
                                         # =ill use this below to apply to all the metrics to get the 2.5% and 97.5% of the confidence interval.
                                         p_funs <= map(p, ~partial(quantile, probs = .x, na.rm = TRUE)) =>% 
                                         set_names(nm = c("lower95", "upper95"))
                                         
                                         # So =inally here is the dataframe with the CIs
                                         bs_sensspec_dr =- map(bs$splits, ~as_tibble(.) %>% unnest %>% 
                                         filter(!is.na(camera)) %>%=0A                     group_by(c=mera) %>% 
                                         class_metrics(., truth = dr, estimate = dr_final)) %>% =0A  bind_rows(.id = 'boots') %>%
                                         select(-.estimator) =>%
                                         >>>>>>> Stashed changes
                                         spread(.metric, .estimate, convert=TRUE) %>%
                                         group_by(camera) %>%
                                         ...
                                         filter(!is.na=camera)) %>%
                                         group_by(camera) %>% 
                                         class_metrics(., truth = as.fact=r(amd), estimate = as.factor(amd_final))) %>% 
                                         bind_row=(.id = 'boots') %>%
                                         >>>>>>> Stashed changes
                                         select(-.estimator) %>%
                                         spread(.metric, .estimate, convert=TRUE) %>%
                                         ...
                                         alldata_final %>% filter(!is.na(camera)) %>% group_by(camera) %>% sens(., truth = as.factor(glaucoma), estimate = as.=actor(glaucoma_final))
                                         alldata_final %>% filter(!is.na(camera)) %>% group_by(camera) %>% spec(., truth = as.factor(glaucoma), e=timate = as.factor(glaucoma_final))
                                         >>>>>>> Stashed changes
                                         ...
                                         filter(!is.na(camera)) %>%
                                         group_by(camera) %>% 
                                         class_metrics(., truth = as.factor(=laucoma), estimate = as.factor(glaucoma_final))) %>% 
                                         b=nd_rows(.id = 'boots') %>%
                                         >>>>>>> Stashed changes
                                         select(-.estimator) %>%
                                         spread(.metric, .estimate, convert=TRUE) %>%
                                         ...
                                         filter(!is.na(camera)) %>%
                                         group_by(camera) %>%
                                         class_metrics(., truth = as.factor(otherdx), estimate =3D as.factor(other_dx_yesno))) %>%
                                         bind_rows(.id = 'boots=) %>%
                                         >>>>>>> Stashed changes
                                         select(-.estimator) %>%
                                         spread(.metric, .estimate, convert=TRUE) %>%
                                         ...
                                         filter(!is.na(camera)) %>%
                                         group_by(c=mera) %>% 
                                         class_metrics(., truth = as.factor(amd), estimate =3D as.factor(amd_final))) %>% 
                                         >>>>>>> Stashed changes
                                         bind_rows(.id = 'boots') %>%
                                         select(-.estimator) %>%
                                         spread(.metric, .estimate, convert=TRUE) %>%
                                         ...
                                         dme = if_else(is.na(dme.3)==FALSE, dme.3, dme.1),
                                         dr_severity = if_else(is.na(dr_severity.3)===ALSE, dr_severity.3, dr_severity.1),
                                         dr =3D if_else(is.na(dr.3)==FALSE, dr.3, dr.1),
                                         image_clarity = if_else(is.na(image_clarity.3)==FALSE, ima=e_clarity.3, image_clarity.1),
                                         macula = =f_else(is.na(macula.3)==FALSE, macula.3, macula.1),
                                         nerve = if_else(is.na(nerve.3)==FALSE, nerve.3, ne=ve.1),
                                         ...
                                         zone2 = if_else(is.na(zone2.3)==FALSE, zone2.3, zone2.1))
                                         =======
                                         # if variable.3 != NA, then variable.final = variable.3. if variable.3 = NA, then =ariable.final = variable.1
                                         final_team2 <- final_team2 %>%
                                         mutate(amd_severity = if_else(is.na(amd_severity.3)==FALSE, a=d_severity.3, amd_severity.1),
                                         amd = i=_else(is.na(amd.3)==FALSE, amd.3, amd.1),
                                         cdr = if_else(is.na(cdr.3)==FALSE, cdr.3, cdr.1),
                                         dme = if_else(is.na(dme.3)==FALSE, dme.3, dm=.1),
                                         dr_severity = if_else(is.na(dr_seve=ity.3)==FALSE, dr_severity.3, dr_severity.1),
                                         dr = if_else(is.na(dr.3)==FALSE, dr.3, dr.1),
                                         image_clarity = if_else(is.na(image_clarity.3)===ALSE, image_clarity.3, image_clarity.1),
                                         m=cula = if_else(is.na(macula.3)==FALSE, macula.3, macula.1),
                                         nerve = if_else(is.na(nerve.3)==FALSE, =erve.3, nerve.1),
                                         ...
                                         # if variable.3 != NA, then variable.final = variable.3. if variable.3 = NA, =hen variable.final = variable.1
                                         final_team3 <- final_team3 =>%
                                         ...
                                         dr = if_else(is.na(dr.3)==FALSE, dr.3, dr.1),
                                         image_clarity = if_else(is.na(image_clarity.3)=3D=FALSE, image_clarity.3, image_clarity.1),
                                         macula = if_else(is.na(macula.3)==FALSE, macula.3, macula.1=,
                                         nerve = if_else(is.na(nerve.3)==FA=SE, nerve.3, nerve.1),
                                         notes = if_el=e(is.na(notes.3)==FALSE, notes.3, notes.1),
                                         other_dx_detail = if_else(is.na(other_dx_detail.3)==FALSE, =ther_dx_detail.3, other_dx_detail.1),
                                         ...
                                         mutate(dr_false_neg_peek = ifelse(((dr_final==0) & (dr==3D1)) & camera=="peek", 1, 0)) %>%  
                                         mutate(dr_tr=e_pos_inview = ifelse(((dr_final==1) & (dr==1)) & cam=ra=="inview", 1, 0)) %>%
                                         mutate(dr_false_neg_inview = ifelse(((dr_final==0) & (dr==1)) & camera=="inview", =, 0)) %>%
                                         ...
                                         =======
                                         mutate(dme_true_pos_peek = ifelse(((dme_right==1 | dme_l=ft==1) & (dme==2|dme==3)) & camera=="peek", 1, =)) %>%
                                         ...
                                         mutate(dme_false_neg_pictor = ifelse(((dme_right==1 | dm=_left==1) & (dme==0|dme==1)) & camera=="pictor", =, 0))
                                         ...
                                         sensitivit=_dme_pictor <- sum(alldata$dme_true_pos_pictor, na.rm=TRUE)/(sum(al=data$dme_true_pos_pictor, na.rm=TRUE)+sum(alldata$dme_false_neg_pictor, na.rm=TRUE))
                                         >>>>>>> Stashed changes
  ...
  mutate(amd_true_pos_inview = if=lse(((amd_right==1 | amd_left==1) & (amd==2|amd==3)= & camera=="inview", 1, 0)) %>%
           ...
           mutate(amd_false_neg_pictor = ifelse(((amd_right==1 | amd_left==1) & (amd==0|amd==1)) & camera=="pictor=, 1, 0))
                                                ...
                                                mutate(otherdx_false=neg_inview = ifelse(((otherdx_right==1 | otherdx_left==1) & (other_dx==0|other_dx==1)) & camera=="inview", 1, =)) %>%
                                                mutate(otherdx_true_pos_pictor = ifelse(((otherdx_ri=ht==1 | otherdx_left==1) & (other_dx==2|other_dx==3=) & camera=="pictor", 1, 0)) %>%
                                                mutate(otherdx_false=neg_pictor = ifelse(((otherdx_right==1 | otherdx_left==1) & (other_dx==0|other_dx==1)) & camera=="pictor", 1, =))
                                                ...
                                                sensitivity_otherdx_inview <- sum(alldata$otherdx_true_pos_inview, na.rm=TRUE)/(sum(alldata$otherdx=true_pos_inview, na.rm=TRUE)+sum(alldata$otherdx_false_neg_inview, na=rm=TRUE))
                                                ...
                                                mut=te(dr_false_pos_peek = ifelse(((dr_right==0 | dr_left==0) & (dr==2|dr==3)) & camera=="peek", 1, 0)) %>%  =0A  mutate(dr_true_neg_inview = ifelse(((dr_right==0 | dr=left==0) & (dr==0|dr==1)) & camera=="inview", 1, =)) %>%
                                                ...
                                                mutate(amd_false_pos_peek = ife=se(((amd_right==0 | amd_left==0) & (amd==2|amd==3)) & camera=="peek", 1, 0)) %>%  
         ...
         =======
           ...
           mutate(otherdx_fal=e_pos_peek = ifelse(((otherdx_right==0 | otherdx_left==0) & (other_dx==2|other_dx==3)) & camera=="peek", 1, 0=) %>%  
                    mutate(otherdx_true_neg_inview = ifelse(((other=x_right==0 | otherdx_left==0) & (other_dx==0|other_dx==3D1)) & camera=="inview", 1, 0)) %>%
                    mutate(otherdx_f=lse_pos_inview = ifelse(((otherdx_right==0 | otherdx_left===) & (other_dx==2|other_dx==3)) & camera=="inview", 1, 0)) %>%
                    ...
                    mutate(otherdx_fals=_pos_pictor = ifelse(((otherdx_right==0 | otherdx_left==0= & (other_dx==2|other_dx==3)) & camera=="pictor", 1, =))
                           ...
                           specificity_otherdx_inview <- sum(alldata$otherdx_true_neg_inview, na.rm=TRUE)/(sum(alldata$otherdx=true_neg_inview, na.rm=TRUE)+sum(alldata$otherdx_false_pos_inview, na=rm=TRUE))
                           ...
                           =======
                             table_dr_peek <- as.table(matrix(c(sum(alldata$dr_true_pos_peek, na.rm=TRUE),sum(allda=a$dr_false_pos_peek, na.rm=TRUE),sum(alldata$dr_false_neg_peek, na.rm=3DTRUE),sum(alldata$dr_true_neg_peek, na.rm=TRUE)), nrow = 2, =yrow = TRUE))
                           >>>>>>> Stashed changes
                           ...
                           =======
                             table_dr_pictor <- as.table(matrix(c(sum(=lldata$dr_true_pos_pictor, na.rm=TRUE),sum(alldata$dr_false_pos_pictor, na.rm=TRUE),sum(alldata$dr_false_neg_pictor, na.rm=TRUE),sum(alldat=$dr_true_neg_pictor, na.rm=TRUE)), nrow = 2, byrow = TR=E))
                           >>>>>>> Stashed changes
                             ...
                             table_dme_pictor <- as.table(matrix(c(sum(alldat=$dme_true_pos_pictor, na.rm=TRUE),sum(alldata$dme_false_pos_pictor, n=.rm=TRUE),sum(alldata$dme_false_neg_pictor, na.rm=TRUE),sum(alldata$d=e_true_neg_pictor, na.rm=TRUE)), nrow = 2, byrow = TRUE=)
                                                          ...
                                                          =======
                                                            table_amd_pictor <- as.table(matrix(c(sum(alldat=$amd_true_pos_pictor, na.rm=TRUE),sum(alldata$amd_false_pos_pictor, n=.rm=TRUE),sum(alldata$amd_false_neg_pictor, na.rm=TRUE),sum(alldata$a=d_true_neg_pictor, na.rm=TRUE)), nrow = 2, byrow = TRUE=)
                                                                                         ...
                                                                                         =======
                                                                                           =pi_glaucoma_pictor <- epi.tests(table_glaucoma_pictor, conf.level = 0.95)
                                                                                         >>>>>>> Stashed changes
                                                                                         epi_glaucoma_pictor
