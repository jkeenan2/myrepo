...
# JM - I tend to use glimpse too. It's faster (I think?) and is good for a quick look at variable names, types, and data structure. 
#      skim is good for getting summaries and some metadata but it won't show you what the actual data looks like the way glimpse does.
#      Really your preference though.
...
# JM - I would make sure these are actually identical, using something like this:
randomcompare %>% mutate(identical=ifelse(randomization_model==referresultnegatives,T,F)) %>% count(identical)
# result: there are actually 3 ids with differences (I think you can ignore the NAs, that's just comparing NA columns)
# of course, you're comparing 2 datasets here and maybe I'm not using the most up-to-date version of both 
...
# JM - I would start by seeing what is being coerced to NA
screeningdata2 %>% 
  mutate(iop_left_eye_os_num=as.numeric(iop_left_eye_os)) %>% 
  filter(is.na(iop_left_eye_os_num)) %>% 
  select(iop_left_eye_os,iop_left_eye_os_num) %>% View()
# JM - looks like the coerced NA was "na" before the change, so this is ok
...
csdatawide <- full_join(screeningdata2, yingdata, by="study_id") %>%
  ...
  mutate(iop.re=as.numeric(iop.re),
         #iop.le=if_else(iop.le=="na","", iop.le), # JM - this variable is already numeric, so it wouldn't have "na" as a value
...
# JM - see my email for general data cleaning advice 
...
csdatalongreferralcheck <- csdatalong %>%
  ...
  ungroup() %>% #JM - if you don't want to include the variable you previously grouped on, you need to add "ungroup()" to your pipe
...
# JM - see my comment at line 511

shouldcompletegs <- csdatalongreferralcheck %>%
  filter(consent==1) %>%
  filter(maxanyfail== 1 |  randomization_model == "Refer") #JM - this line wasn't connected to your pipe so I added %>% to the end
#of the line above. Did you want me to do this? 

accountedfor <- csdatalongreferralcheck %>%
  filter(ophthalmologist_exam_complete==2 | phone_call_complete==2) 
# JM - ^you're filtering on variables you removed from csdatalongreferralcheck in line 512 so this won't run
...
photoagreement <- full_join(csdatawide, jeremyreview, by="study_id") %>%
  ...
  mutate(start_time_inclusion=ymd_hms(start_time_inclusion)) # JM
# 
# #? unsure why this is lubridate function is not working. 

# JM - if you're trying to modify the start_time_inclusion variable with lubridate, you need to put this into a mutate() 

# #I am trying to separate out the date to give month by month, but will leave out for now

# JM - here is some code to get months: 
photoagreement_JM <- photoagreement %>% 
  mutate(year=year(start_time_inclusion),
...
# JM - and if you want monthly kappas:
allyrmo <- photoagreement_JM %>% select(yr_mo) %>% distinct() %>% pull()
# ^ this will create a list of all month-year combos from the above dataset
allyrmo # I noticed it's only for Sep-2017 to Jan-2018 (with 1 possible data entry error at Oct-2018)
# I'm assuming that you stopped doing QC around the end of 2017/beginning of 2018

# JM - this is a for loop that will run through calculating kappas at each month
kappalist=data.frame()
for (value in allyrmo){
  kappa <-(kappa2(photoagreement_JM %>% filter(yr_mo==value) %>% select(photoscreenpos,this_form_referral), weight = "unweighted", sort.levels = FALSE))[5]
  n <-(kappa2(photoagreement_JM %>% filter(yr_mo==value) %>% select(photoscreenpos,this_form_referral), weight = "unweighted", sort.levels = FALSE))[2]
...
# JM - regarding the following, from my understanding a kappa b/w 0.4 and 0.6 is "moderate" 
...
# JM - I'm assuming this question was more directed at JK. I've never used this package before (cool though). 
#      In my experience I've used powerpoint, Adobe products, etc. to do this sort of thing. 
...
# JM - Sorry, what is the issue? Are you saying the denoms for glc suspect and any glc should be about the same? 
# UPDATE - JM - After looking into this more: Go back to lines 386-390. When you define the oe_anyglcsuspect variable, you base it
# off of oe_vcdr which has 1294 missing values. Because oe_anyglcsuspect is based on this variable, you end up creating
# lots of NAs that don't exist in oe_anyglc. Therefore, you have a much lower denominator for oe_anyglcsuspect when you exclude
# NA values. I would go back and make sure this is all defined correctly. (For example, are the 1294 missing oe_vcdr values 
# actually missing, or are they a calculation error) 
...
table4long <- table4wide %>% # JM - having trouble with this table. dmclinic isn't defined in table4wide?
  select(-(consent_n)) %>%
...
###########################################################################
## DATA CLEANING FOR BLAKE...COMPLETED
